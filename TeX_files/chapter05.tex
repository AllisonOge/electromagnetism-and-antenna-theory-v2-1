...
\end{equation*}\\
 \begin{equation*}
	...
	\section{Voltage And Current Variation on Transission Line}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.7\linewidth]{11111111}
...
As we move from load to generator, $l$ increases positively, making $\phi_L -2 \beta l$ more and more negative. In the complex plane, when a phase gets more negative, it means we are moving in the clockwise direction.
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.9\linewidth]{"473 drawings"}
		\caption{Clockwise and Anticlockwise movement of l}
		\label{fig:473-drawings}
	\end{figure}
	
So by moving towards the generator, the phase becomes more and more negative, while the amplitude of $|\Gamma_L|$ remains constant. The total voltage is scaled by the vector sum of real term (1) and complex term $|\Gamma_L|  e^{j(\phi_L -2 \beta l)}$. Hence, we have a summation of real vector, whose magnitude is 1, plus a complex term shown below. 
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.6\linewidth]{kjhgfdwert}
...
\\ Also, if $\phi_L - 2 \beta l=(2 n + 1) \pi$, then $e^{j (\phi_L - 2 \beta l)} = -1$. This is the left side of the dot on the real axis.
	 ...
	 As stated earlier, when $ e^{j(\phi_L - 2 \beta l)} = 1$, the voltage is maximum and the current is minimum. When $ e^{j(\phi_L - 2 \beta l)} = -1$ , the voltage is minimum and the current is maximum. So at same location along the transmission line, maximum voltage corresponds to minimum current, and minimum voltage corresponds to maximum current. This is opposite to what we observed in a lumped circuit, where maximum voltage point corresponds to maximum current and minimum voltage corresponds to minimum current.\\
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.6\linewidth]{asdfghjhgfdsa}
	...
	\end{equation}\\
	At location of minimum voltage\\
	Minimum Impedance,
	\begin{equation}
...
Now, once we have the voltage standing wave on the transmission line, at high frequencies the measurement of phase is very difficult and complicated. One can measure the amplitude of a signal reliably but the measurement of phase is rather uncertain. So at high frequency, we estimate the phase not in a direct manner, but in an indirect manner by carrying out the measurement of only magnitude kind of quantities, we would like to estimate the phase of the signal. As we have seen the phase of the signal in time gets translated into the space phase because the total phase we see on a wave is a combination of space and time or the phase relationship between the two waves, the forward and backward wave which is related to the time phase and the space phase ($\omega t + \beta x$). Where;
	...
	\end{equation*}
	This makes sense because $|\Gamma_L|$ is the relative amplitude of the reflected wave with respect to the incident wave. Since we do not have any energy source at the load point, part of the energy can only be reflected (energy supplied by generator). So the amplitude of the reflected wave has to always be less than or equal to the incident wave.\\\\
	Conditions for which $|\Gamma_L|=1$ :\\\\
	$\boldsymbol{Case 1}:$
	\\$Z_L=0$ : the line is short circuited at the load end.
	\begin{equation*}	
	\Gamma_L= \frac{Z_L -Z_o}{Z_L + Z_o} =  \frac{0 -Z_o}{0 + Z_o} = -1	
	\end{equation*}
	\begin{equation*}
	|\Gamma_L|=1
	\end{equation*}\\	
	$\boldsymbol{Case 2}:\\
	$ $Z_L=\infty$: Open circuit line.
	\begin{equation*}
	\Gamma_L= \frac{Z_L -Z_o}{Z_L + Z_o}
	\end{equation*}
	\begin{equation*}
	\Gamma_L = \frac{1 -\frac{Z_o}{Z_L}}{1 + \frac{Z_o}{Z_L}} = \frac{1 - 0}{1 + 0} = +1
	\end{equation*}
	\begin{equation*}
	|\Gamma_L| = 1
	\end{equation*}\\
	$\boldsymbol{Case 3}:\\
	$ $Z_L = jX$: Pure reactance.
	\begin{equation*}
	\Gamma_L= \frac{Z_L -Z_o}{Z_L + Z_o}
	\end{equation*}
	\begin{equation*}
	\Gamma_L = \frac{jX - Z_o}{jX + Z_o} = complex
	\end{equation*}
	\begin{equation*}
	|\Gamma_L| = 1
	\end{equation*}\\\\
	So the three cases under which $|\Gamma_L|=1$ is;\\
	(i)		When the line is short circuited.\\
	(ii)	When the line is open circuited.\\
	(iii) 	When the line is terminated in a pure reactance, there will be full reflection from the load end.\\ 
	Since there is no energy-absorbing circuit at the load end of the line, short circuit, open circuit and ideal reactance cannot absorb power. So, whatever power the generator takes to the load end, it has no option but to return all the power back in the reflected wave form.
	\begin{align*}
	VSWR  = \frac{1+|\Gamma_L|}{1-|\Gamma_L|}
	\end{align*}
	 will aways be greater than 1, the best case is $|\Gamma_L|=0$ and $VSWR=1$.
	$|\Gamma_L|=1$, $\rho= \infty$. So $1\leq\rho\leq\infty$\\\\
	$|\Gamma_L|=0$ represents no reflected wave on the transmission line i.e full power is transfered to the load which means VSWR=1 which is the best case for power transfer on the line. As the VSWR increases, it creates higher value of $|\Gamma_L|$, which means more reflection on the transmission line or less efficiency of power transfer to the load. So, every circuit design try to make VSWR as small as possible or as close to 1 as possible. Higher value of VSWR indicates more mismatch on the transmission line, or higher value of the reflected wave on the transmission line.\\
	So VSWR is one of the most important quantity at high frequency. When designing a circuit, we try to make sure VSWR on the transmission line is close to one as possible, making sure the circuit is efficiently transferring power to the load end of the line. Once $\rho$ or VSWR is defined, we can relate it to maximum and minimum impedance that can be seen on the transmission line.\\
