...
Given $Z_L$ or $Y_L$, if we are to find $Z_L$ or $Y_L$ at distance $l$ from the load, we use the hypothetical sketch of the smith chart shown in figure~\ref{fig:uytrewsxcvbj} below;
\begin{figure}[h]
\centering
...
\begin{equation}
\overline{R}_{max} = {\rho}
\end{equation}
and
\begin{equation}
\overline{R}_{min} = \frac{1}{\rho}
\end{equation}
