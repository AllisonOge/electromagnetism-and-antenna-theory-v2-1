...
\end{align}
From equations \eref{eqn:wavemagincid} and \eref{eqn:wavemagref}, we can rewrite $H_i$ and $H_r$ in equations \eref{eqn:magincid} and \eref{eqn:magref} in terms of $E_i$ and $E_r$ respectively;
\begin{align}
...
Substitute $E_i$ = - $E_r$ and add equations \eref{eqn:elecincid} and \eref{eqn:elecref}
\begin{align}
...
A complex wave is the combination of a standing wave which is in a direction perpendicular to the plane/boundary and a travelling wave which is in the direction of the plane.
...
equation~\eref{eqn:totalmagfieldx2}
\begin{align}
...
From that analogy, $e^{j\beta xcos \theta} - e^{-j\beta xcos\theta} = 2cos\beta xcos\theta$ and substitute into equation~\eref{eqn:totalmagfieldz}
\begin{align}
H_z = \frac{-2E_i}{\eta}(cos\theta(cos\beta xcos\theta) (e^{-j \beta zsin\theta})
\label{eqn:totalmagfieldz2}
\end{align}
From equation~\eref{eqn:totalmagfieldz2}, we see that the z component of the magnetic field Hz has a standing wave component $(cos\beta xcos\theta)$ and a travelling wave component $(e^{-j\beta zsin\theta})$. So in general, the fields in the dielectric medium are a combination of traveling wave and standing wave.
