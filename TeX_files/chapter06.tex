...
\begin{align*}
\footnotemark Z(l) = Z_0\left\lbrace \frac{Z_L\cosh(\gamma l) + Z_0\sinh(\gamma l)}{Z_0\cosh(\gamma l) + Z_L\sinh(\gamma l)}\right\rbrace 
\end{align*}
\footnotetext{
$Z(l)$ is the impedance at any point on the line
}

\begin{align*}
\footnotemark\bar{Z}(l) = \left\lbrace \frac{\bar{Z}_L\cosh(\gamma l) + \sinh(\gamma l)}{\bar{Z}_L\sinh(\gamma l) + \cosh(\gamma l)}\right\rbrace 
\end{align*}
\footnotetext{
$\bar{Z}(l)$ is the normalized impedance at any point on the line
}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.1\linewidth]{\pathtopartone/graphics/load_over_half_wavelength}
...
Then at location ${(l+\frac{\lambda}{4})}$, Impedance $=\bar{Z}(l+\frac{\lambda}{4})$. Substituting ${(l+\frac{\lambda}{4})}$ for $l$ in Equation~\eqref{eqn:implossless}, we get:
\begin{dmath*}
...
These are the three very important characteristics of a lossless transmission line.
\begin{enumerate}[(i)]
...
\end{dmath*}
Recall, \( j2\sin\theta = {e}^{j\theta} - {e}^{j\theta}\), so,
\begin{dmath} 
...
\begin{figure}[h]
\centering
\includegraphics[width=1.1\linewidth]{\pathtopartone/graphics/load_transformation}
...
\end{equation}
So solving Equation~\eqref{eqn:evaluatev+current} making $V^{+}$ the subject of the formula, we get the expression of $V^+$ as follows. 
\begin{equation} 
...
\subsection*{Summary}
Table~\ref{tab:transparams} summarizes the transmission line parameters for lossless and low-loss conditions.

\captionof{table}{Transmission line parameters for lossless and low-loss conditions}
\label{tab:transparams}  
\begin{tabular}{|l|l|}
\hline
\textbf{Lossless} & \textbf{Lowloss} \\ \hline
$\alpha = 0$ & $\alpha = \frac{1}{2}R\sqrt{\frac{C}{L}} + \frac{1}{2}G\sqrt{\frac{C}{L}}$\\
...
\captionof{figure}{Antenna configuration for Ex.~\ref{ex62}}
