...
\section{Quarter Wavelength Transformer Technique}
 ...
 \begin{figure}[h]
 	\centering
 	\includegraphics[width=1\linewidth]{smich}
 	\caption{Smith chart}
 \end{figure}
...
\[
 ...
 What does this tell us? This equation tells us that if we have a mismatch condition, we can input a matching transformer ( an example is the BALUN)\footnote{A BALUN is a device which is use to connect balance load to an unbalance load, it is mostly use in television.  it has a length ($L=\lambda/4$), and a characteristic impedance $Z_{ox} $}.
 	...
 	\end{figure}
 	  ...
 	  From figure 12.5, we have to move $ Z$ which is complex by a distance l on the transmission line to make its transformed impedance real at point B.\\%put figure here smith chart 12.6%
 	 \begin{figure}[h]
 	 	\centering
 	 	\includegraphics[scale=0.6]{sinsmith}
...
\end{itemize} 
\subsection{Single Stub Matching Technique }% put single stub matching figure here%
 \begin{figure}[h]
	\centering
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{qwtch}
...
\begin{figure}[h]
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=1\linewidth]{dousmith}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{question1}
