...
\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{./graphics/fig3.3}
...
\begin{exmp}
	...
	$ \omega = 2\pi \times 10^9 $ rad/s.\\\\
	Substituting into the $ \gamma $ expression; \\\\
	$\sqrt{[0.4+\jmath (2\pi \times 10^9)\times 0.3 \times 10^{-6}][0.08 + \jmath(2\pi \times 10^9) \times 120 \times10^{-12}]}  $\\\\
	$ \gamma = \sqrt{(0.5 + \jmath 600\pi)(0.08 + \jmath 0.24\pi)} $\\\\
	Expanding,\\\\
	$ = \sqrt{0.4(0.08) + 0.4(\jmath 0.24\pi) + \jmath 600\pi(0.08) + \jmath 600\pi(\jmath 0.24\pi)} $ \\\\
	Recall  that,$ \quad \jmath \times \jmath = \sqrt{-1} \times \sqrt{-1} = (\sqrt{-1})^2 = -1 $\\\\
	$ =\sqrt{0.032 + \jmath 0.3016 +\jmath 150.7964 - 1421.2230} $ \\\\
	$ = \sqrt{-1421.191 + \jmath 151.098} $ \\
	
	We first convert the complex number to polar form.\\\\
	-$ -1421.191 + \jmath 151.098 = 1429.2006\angle 173.931\textdegree $\\\\
	$ \sqrt{-1421.191 + \jmath 151.098} = \sqrt{1429.2006}\angle \frac{173.931}{2} $\\\\
	$ =37.8\angle 86.9655 $\\\\
	Converting back to cartesian form; \\\\
	Propagation constant,$\quad\gamma=2 +\jmath 37.747 $\\\\
	From the expression; \\
	$ \alpha = 2 $ Nepers/m, $ \beta = 37.747 $ rad/m\\\\
	We now convert the attenuation constant to dB/m; \\
	1 Nepers/m = 8.68dB/m \\\\
	2 Nepers/m = 17.36 dB/m
...
\section*{QUESTIONS}
\begin{enumerate}
	\item With appropriate equation of amplitude variation along a transmission line, explain what is meant by Characteristic length.
	
	\item In what unit is attenuation constant stated in most data sheets?
	
	\item Starting from the differential equation guilding transmission lines, derive the characteristic impedance relation. What does negative impedance imply from the backward voltage to current ratio?
	
	\item  What are the physical significance of the propagation constant($\gamma$), attenuation constant($\alpha$) and phase constant($\beta$)?
	
	\item From $ V = V^+e^{+\lambda l} + V^-e^{-\lambda l} $ and $ I = \frac{V^+}{Z_0}e^{+\lambda l} - \frac{V^-}{Z_0}e^{-\lambda l}$, show that $ \Gamma_L = \frac{Z_L - Z_o}{Z_L + Z_o} $.
